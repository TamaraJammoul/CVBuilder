...
exports.addCV = (req, res) => {
  User.findOne({Email: req.body.Email}).exec(async (err, user) => {
    ...
    cv.save().then((savedCV) => {
      let tmpCV = user.CV;
      if (tmpCV === null) tmpCV = [];
      tmpCV.push(savedCV._id);
      user.updateOne({CV: tmpCV}).then(() => {
          ...
          Experience.deleteMany({_id: {$in: cv.Experiences}}, (err) => {
            ...
            Language.deleteMany({_id: {$in: cv.Languages}}, (err) => {
              ...
              Memberships.deleteMany({_id: {$in: cv.Memberships}}, (err) => {
                ...
                OtherTraining.deleteMany(
                  {_id: {$in: cv.OtherTrainings}},
                  (err) => {
                    if (err) {
                      return res.status(400).json({
                        err,
                    ...
                    PersonalInformation.deleteOne(
                      {_id: cv.PersonalInformation},
                      (err) => {
                        if (err) {
                          return res.status(400).json({
                            err,
                        ...
                        TechnicalSkills.deleteMany(
                          {_id: {$in: cv.TechnicalSkills}},
                          (err) => {
                            if (err) {
                              return res.status(400).json({
                                err,
                                                  ...
                                                  if (user) {
                                                    ...
                                                    User.updateOne(
                                                      ...
                                                      return res
                                                        ...
                                                        .json({
                                                          msg:
                                                            "delete process completed successfuly",
                                                        });
                                                    });
                                                  }
                                                });
